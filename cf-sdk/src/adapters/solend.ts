...
export class SolendReserveAsset extends Asset {
  ...
  static async initialize(
    ...
    const marketKeypair = await initLendingMarket(provider, owner.publicKey, wallet.payer, pythProgram, switchboardProgram);
...
export async function addReserve(
  ...
  const [lendingMarketAuthority] = await PublicKey.findProgramAddress([lendingMarket.toBuffer()], DEVNET_PROGRAM_ID);
